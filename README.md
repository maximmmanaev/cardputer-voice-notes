...
Автономный диктофон для **M5Stack Cardputer-Adv**. Устройство пишет честный PCM-звук на microSD, а в знакомой Wi-Fi-сети передаёт закрытые фрагменты локальному macOS-agent. Agent проверяет WAV, конвертирует его в OGG/Opus и отправляет обычным Telegram voice от пользовательской сессии в выбранную группу. Основной сценарий не использует Telegram Bot API.
...
- **Fn + `** (клавиша `Esc`): отменить текущую синхронизацию. Запись остаётся на карте, частично принятый файл остаётся на Mac и продолжится с сохранённого offset при следующей попытке.
...
Скрипт скрыто запросит SSID и пароль, сгенерирует общий device token, определит LAN IP Mac и создаст два файла с правами `0600`: `.env` и игнорируемый `firmware/include/local_config.h`. Секреты не печатаются.
...
- `SYNC CANCELLED`: нажать `S` позже; передача продолжится с сохранённого offset.
